...
async def get_valid_instrument_tickdata(
    ...
    try:
        ...
        if not df.empty:
            # Use instrument_seq (integer) as instrument_id for TimescaleDB hypertable
            df["instrument_id"] = int(row.instrument_seq)
...
def write_to_sql_with_progress(
    ...
    """Write a DataFrame to SQL database in chunks with dedup via staging table.
    
    Uses a staging table + INSERT ... ON CONFLICT DO NOTHING pattern to ensure
    idempotent inserts (safe to re-run for the same date without creating duplicates).
    Falls back to simple append if no unique constraint exists yet.
    """
    if df.empty:
    ...
    try:
        # Check if unique constraint exists on the target table
        has_unique_constraint = False
        with engine.connect() as check_conn:
            result = check_conn.execute(text(
                f"SELECT 1 FROM pg_constraint "
                f"WHERE conrelid = '{schema}.{table_name}'::regclass "
                f"AND contype = 'u' LIMIT 1"
            ))
            has_unique_constraint = result.fetchone() is not None
        
        if has_unique_constraint:
            # Use staging table + ON CONFLICT DO NOTHING for idempotent inserts
            logger.info("Using staging table approach (unique constraint detected)")
            staging_table = f"_staging_{table_name}"
            columns = ", ".join(df.columns)
            
            with engine.begin() as conn:
                # Create temp staging table matching target structure
                conn.execute(text(
                    f"CREATE TEMP TABLE {staging_table} (LIKE {schema}.{table_name} INCLUDING DEFAULTS) "
                    f"ON COMMIT DROP"
                ))
                
                # Write data to staging table in chunks
                for start in tqdm.tqdm(
                    range(0, len(df), chunksize),
                    total=(len(df) // chunksize) + 1,
                    desc=f"Staging {table_name}",
                ):
                    ...
                    logger.debug(f"Staged chunk {start//chunksize + 1}: {len(chunk)} records")
                
                # Insert from staging to target with conflict handling
                result = conn.execute(text(
                    f"INSERT INTO {schema}.{table_name} ({columns}) "
                    f"SELECT {columns} FROM {staging_table} "
                    f"ON CONFLICT (instrument_id, time_stamp) DO NOTHING"
                ))
                inserted = result.rowcount
                skipped = len(df) - inserted
                logger.info(f"Inserted {inserted:,} new records, skipped {skipped:,} duplicates")
        else:
            # No unique constraint yet - use simple append with Python-level dedup
            logger.info("No unique constraint detected - using simple append with Python dedup")
            
            # Deduplicate within the DataFrame itself
            before_dedup = len(df)
            df = df.drop_duplicates(subset=["instrument_id", "time_stamp"], keep="first")
            after_dedup = len(df)
            if before_dedup != after_dedup:
                logger.info(f"Removed {before_dedup - after_dedup} in-batch duplicates")
        ...
        logger.info(f"Write performance: {records_per_second:.0f} records/second ({write_time:.1f}s)")
...
async def main(date_strs, force=False):
    ...
    try:
        ...
        # Store new instruments FIRST (needed to get instrument_seq for ticker_ts)
        ...
        # Load instrument_seq mapping from DB for ALL instruments
        # instrument_seq is the BIGSERIAL surrogate key used by the TimescaleDB hypertable (ticker_ts)
        logger.info("Loading instrument_seq mapping from database...")
        seq_map = pd.read_sql(
            "SELECT id, instrument_seq FROM options.instrument", engine
        )
        instrument_df = instrument_df.merge(seq_map, on="id", how="left")

        # Verify all instruments have instrument_seq
        missing_seq = instrument_df["instrument_seq"].isna().sum()
        if missing_seq > 0:
            logger.warning(f"{missing_seq} instruments missing instrument_seq - these will be skipped")
            instrument_df = instrument_df.dropna(subset=["instrument_seq"])

        instrument_df["instrument_seq"] = instrument_df["instrument_seq"].astype(int)
        logger.info(f"Loaded instrument_seq for {len(instrument_df)} instruments")
        ...
        # Store ticker data to TimescaleDB hypertable
        if not ticker_df.empty:
            logger.info("Storing ticker data to ticker_ts...")
            # Ensure correct dtypes for TimescaleDB hypertable
            ticker_df["instrument_id"] = ticker_df["instrument_id"].astype(int)
            write_to_sql_with_progress(ticker_df, "ticker_ts", engine)
        else:
